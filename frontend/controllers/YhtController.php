    ...
    public function actionContractCreate($tid=null){
        ...
        if ($tid!=null){
            ...
            if (empty($contractInfo)){
                ...
                //自动添加合同签署人
                $client->sendReq('post',YhtClient::$url['contract']['sign'],["idType"=>0,"idContent"=>$contractId,"signerId"=>$signerId]);
            }else{
...
//        return $this->render('contract-detail',['contractId'=>1008611,'status'=>0,'isOwner'=>1]);die;
        ...
        if (!empty($contractInfo)){
            ...
            if ($contractInfo['cont_has_bind']==2){
                ...
                try{
                    //进行锁定,插入乙方记录
                    Yii::$app->db->createCommand()->insert('yii2_wx_yht_contract_signer',['contract_id'=>$contractId,'user_id'=>$user_id])->execute();
                    //更新合同锁定状态
                    Yii::$app->db->createCommand()->update('yii2_wx_yht_contract',['cont_has_bind'=>2],['contract_id'=>$contractId])->execute();
                }catch (\yii\db\Exception $e){
                    p("抱歉! 系统出错,请刷新再试",1);
                }
            }

            $status = $contractInfo['cont_status'];
            $isOwner = $contractInfo['cont_owner_signerId']==$_SESSION['userinfo']['yht_signerId'];
            return $this->render('contract-detail',['contractId'=>$contractId,'status'=>$status,'isOwner'=>$isOwner]);
        }else{
            return $this->render('contract-detail',['contractId'=>'1807261536209921','status'=>0,'isOwner'=>0]);
            p("抱歉你所查看的合同不存在",1);
        }
    }
...
//        output::ajaxReturn(200,'success');
        if (Yii::$app->request->isAjax){
            ...
            try{
                ...
                if ($res['code']==200){
                    //保存到数据库
//                    Yii::$app->db->createCommand()->update('yii2_wx_yht_info',['yht_moulage_base64'=>$res['data']['moulage']['imgBase']])->execute();
